...
JS);
    ...
    protected function buildJsDataLoaderParams(string $oControlEventJsVar = 'oControlEvent', string $oParamsJs = 'params', $keepPagePosJsVar = 'bKeepPagingPos') : string
    {
        return $this->buildJsDataLoaderParamsViaTrait($oControlEventJsVar, $oParamsJs, $keepPagePosJsVar) . <<<JS
            ...
            $.extend($oParamsJs, oLeafletParams);
JS;
    }
...
JS;
    ...
    protected function buildJsLeafletDataLoader(DataMapLayerInterface $layer, string $oRequestParamsJs, string $aResultRowsJs, string $onLoadedJs) : string
    {
        ...
        if (! $layerDataWidget->getMetaObject()->is($mapWidget->getMetaObject())) {
            ...
            $oRequestParamsJs = "$.extend({$oRequestParamsJs}, {data: {$layerConfiguratorEl->buildJsDataGetter($readAction, true)}})";
        }
                ...
                if ($link) {
                    ...
                    if ($linked_element) {
                        if ($layer instanceof DataSelectionMarkerLayer) {
                            ...
                            $linked_element->addOnRefreshScript($this->buildJsLeafletRefresh($layer, 'linked_data_changed'));
                        }
                    }
                }
            }
